...
def check_eula_accepted() -> bool:
    ...
    For Free tier: EULA is accepted implicitly by using the software.
    For paid tiers: explicit acceptance is required during activation.
    Returns True always for Free tier (no gate).
    """
    return True
...
class _EULAGroup(click.Group):
    """Custom Click group — Free tier has no EULA gate."""

    def invoke(self, ctx: click.Context) -> None:  # type: ignore[override]
        args = sys.argv[1:]
        # --accept-eula still works for explicit acceptance (paid tiers)
...
@click.version_option(package_name="uaml-memory")
def cli(accept_eula: bool = False):
    ...
    try:
        from uaml.update_checker import check_for_update
        check_for_update(quiet=False)
    except Exception:
        pass  # Never fail on update check

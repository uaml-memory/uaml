...
class MemoryStore:
    ...
    def search(
        ...
        *,
        ...
        # Default: scope to last 30 days for performance on large DBs
        # (unless explicit temporal filter provided)
        if not point_in_time and not any("created_at" in p for p in where_parts):
            where_parts.append("k.created_at > datetime('now', '-30 days')")

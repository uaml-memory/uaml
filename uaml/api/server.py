...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            elif path == "/api/v1/anonymize":
                self._handle_anonymize(body)

            elif path == "/api/v1/deanonymize":
                self._handle_deanonymize(body)
    ...
    # Shared anonymizer instance (lazy-initialized, thread-safe)
    _anonymizer = None

    @classmethod
    def _get_anonymizer(cls):
        if cls._anonymizer is None:
            from uaml.security.anonymizer import TokenAnonymizer
            import os
            db_path = os.environ.get(
                "UAML_ANONYMIZER_DB",
                os.path.expanduser("~/.uaml/anonymizer.db"),
            )
            os.makedirs(os.path.dirname(db_path), exist_ok=True)
            cls._anonymizer = TokenAnonymizer(db_path=db_path)
        return cls._anonymizer

    def _handle_anonymize(self, body: dict):
        """POST /api/v1/anonymize — anonymize text, return anonymized text + mapping.

        Request:  {"text": "Server at 203.0.113.42 user alice"}
        Response: {"text": "Server at $IP1 user $USER1", "mapping": {"$IP1": "203.0.113.42", "$USER1": "alice"}}
        """
        text = body.get("text", "")
        if not text:
        ...
        # Optional: register names on the fly
        names = body.get("names", [])
        anon = self._get_anonymizer()
        if names:
            anon.register_names(names)

        anonymized_text, mapping = anon.anonymize(text)
        self._json_response({
            "text": anonymized_text,
            "mapping": mapping,
        })

    def _handle_deanonymize(self, body: dict):
        """POST /api/v1/deanonymize — reverse anonymization using mapping.

        Request:  {"text": "Server at $IP1 user $USER1", "mapping": {"$IP1": "203.0.113.42", "$USER1": "alice"}}
        Response: {"text": "Server at 203.0.113.42 user alice"}
        """
        text = body.get("text", "")
        ...
        anon = self._get_anonymizer()
        result = anon.deanonymize(text, mapping)

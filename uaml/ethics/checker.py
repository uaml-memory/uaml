...
DEFAULT_RULES = [
    ...
    EthicsRule(
        ...
        pattern=r"(/root/\.agent-data|/home/\w+/\.agent-data|/etc/shadow|/etc/passwd)",
        severity=Severity.SOFT,
        action=Action.FLAG,
        tier=AsimovTier.COMMAND,
        scope="output",
...
class EthicsChecker:
    ...
    def check_input(self, content: str) -> dict:
        """Check incoming content — returns simple dict for bridge integration.

        Returns dict with 'status' (APPROVED/FLAGGED/REJECTED), 'violations', and 'tier'.
        For the full EthicsVerdict object, use check_input_verdict().
        """
        violations = []

        # T0: HARD BLOCK — passwords, tokens, private keys
        if re.search(r'(?i)(password|passwd|heslo)\s*[:=]\s*\S+', content):
            violations.append(('T0', 'password_detected'))
        if re.search(r'(?i)(api[_-]?key|secret[_-]?key|access[_-]?token)\s*[:=]\s*\S+', content):
            violations.append(('T0', 'api_key_detected'))
        if re.search(r'sk-[a-zA-Z0-9]{20,}', content):
            violations.append(('T0', 'openai_key_detected'))
        if re.search(r'-----BEGIN\s+(RSA\s+)?PRIVATE\s+KEY-----', content):
            violations.append(('T0', 'private_key_detected'))

        # T1: HARD BLOCK — health data, national IDs
        if re.search(r'(?i)(diagnos|blood\s+pressure|prescribed|patient|diabetes|cancer)', content):
            violations.append(('T1', 'health_data'))
        if re.search(r'\b\d{6}/\d{3,4}\b', content):  # Czech rodné číslo
            violations.append(('T1', 'national_id'))

        # T2: FLAG — credit cards, excessive PII
        if re.search(r'\b\d{4}[-\s]?\d{4}[-\s]?\d{4}[-\s]?\d{4}\b', content):
            violations.append(('T2', 'credit_card'))

        if any(tier == 'T0' or tier == 'T1' for tier, _ in violations):
            return {'status': 'REJECTED', 'violations': violations, 'tier': violations[0][0]}
        elif violations:
            return {'status': 'FLAGGED', 'violations': violations, 'tier': 'T2'}
        return {'status': 'APPROVED', 'violations': [], 'tier': 'T3'}

    def check_input_verdict(self, content: str) -> EthicsVerdict:
        """Check incoming content (before storage). Applies rules with scope 'input' or 'both'.
        Returns full EthicsVerdict object."""

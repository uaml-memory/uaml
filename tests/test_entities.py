...
class TestEntityExtraction:
    ...
    def test_extract_ip(self, extractor):
        entities = extractor.extract("Server at 192.168.1.187:3003")
    ...
    def test_extract_name_heuristic(self, extractor):
        entities = extractor.extract("Pavel Zamazal decided to use UAML")
        names = [e for e in entities if e.entity_type == "name"]
        assert any("Pavel" in e.text for e in names)
    ...
    def test_summarize(self, extractor):
        text = "Deploy v1.0.0 to 192.168.1.100 — see https://docs.uaml.ai"

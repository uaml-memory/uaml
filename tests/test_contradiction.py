...
class TestFactExtraction:
    ...
    def test_extract_ip(self, checker):
        claims = checker.extract_claims("server-a IP: 192.168.1.10")
        assert any(c.claim_type == "ip" and c.value == "192.168.1.10" for c in claims)
...
class TestContradictionDetection:
    ...
    def test_ip_change_detected(self, store):
        ...
        assert "192.168.1.20" in result.details[0] or "192.168.1.30" in result.details[0]
    ...
    def test_no_conflict_different_subject(self, store):
        ...
        store.learn(
            "ServerA IP: 192.168.1.40",
            topic="infrastructure",
        )
        
        checker = ContradictionChecker(store)
        result = checker.check(
            "ServerB IP: 192.168.1.30",
            topic="infrastructure",
        )
    ...
    def test_status_change(self, store):
        ...
        store.learn(
            "Ollama is running on server-a",
            topic="ollama",
        )
        
        checker = ContradictionChecker(store)
        result = checker.check(
            "Ollama is stopped on server-a",
            topic="ollama",
        )
...
class TestAutoSupersession:
    ...
    def test_superseded_entries_excluded_from_search(self, store):
        ...
        store.learn("database IP: 192.168.1.20", topic="mynode")
        store.learn("database IP: 192.168.1.30", topic="mynode")
        
        # Both should be in DB, but old one is superseded
        superseded = store.get_superseded()
        assert len(superseded) > 0
        assert any("192.168.1.20" in s["content"] for s in superseded)
...
class TestRealWorldScenario:
    """Test the exact scenario from our bug report (merged from integration tests)."""
...
class TestMultipleContradictions:
    """Test handling of multiple contradictions at once (from integration tests)."""
...
class TestTemporalAfterSupersede:
    """Test point-in-time queries after supersession (from integration tests)."""
...
class TestEdgeCases:
    """Edge cases and boundary conditions (merged from integration tests)."""
